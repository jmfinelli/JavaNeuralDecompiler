...
public class JuicerRecordMatchBasedTransformer<LOW_ITEM> implements Function<
    ...
    public JuicerRecordMatchBasedTransformer(
    ...
    public Stream<DecompilationRecord<MethodJuice<LOW_ITEM>, MethodJuice<MethodDeclaration>>> apply
        ...
        Map<String, InfoExtractor.InfoType> highLevelSet = highLevelInfoExtractor.apply(decompilationRecord.getHighLevelRepresentation());

        Map<String, String> placeholders = new HashMap<>();

        for (Map.Entry<String, InfoExtractor.InfoType> entry : lowLevelSet.entrySet()) {
            if (highLevelSet.containsKey(entry.getKey())) {
